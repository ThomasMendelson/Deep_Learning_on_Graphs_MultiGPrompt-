...
class downprompt(nn.Module):
    def __init__(self, prompt1, prompt2, prompt3,a4,ft_in, nb_classes, feature, labels):
        ...
        self.aveemb0 = torch.FloatTensor(ft_in, ).to(device)
        self.aveemb1 = torch.FloatTensor(ft_in, ).to(device)
        self.aveemb2 = torch.FloatTensor(ft_in, ).to(device)
        self.aveemb3 = torch.FloatTensor(ft_in, ).to(device)
        self.aveemb4 = torch.FloatTensor(ft_in, ).to(device)
        self.aveemb5 = torch.FloatTensor(ft_in, ).to(device)
        self.aveemb6 = torch.FloatTensor(ft_in, ).to(device)


        self.one = torch.ones(1,ft_in).to(device)
        ...
        self.ave = torch.FloatTensor(nb_classes,ft_in).to(device)
    ...
    def forward(self,seq,seq1,train=0):
        ...
        # Check if CUDA is available
        # print("rawret",rawret)
        # print("rawret.shape",rawret.shape)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        rawret = rawret.to(device)
        ...
        ret = torch.FloatTensor(seq.shape[0],self.nb_classes).to(device)
        # print("avesize",self.ave.size(),"ave",self.ave)
        # print("rawret=", rawret[1])
        # print("aveemb", self.ave)
        for x in range(0,seq.shape[0]):
           # print("rawret[x]",rawret[x])
            # print("rawret[x].shape",rawret[x].shape)
    ...
    def embedding(self,seq,seq1,train=0):
        # added fot progject -> This function get all prompts of nodes and returns thier embeddings
        weight = self.leakyrelu(self.nodelabelprompt(self.prompt))
        weight = self.one + weight
        # weight = torch.mm(promptweight,self.prompt)
        # # print("weight",self.a.weight)
...
def averageemb(labels,rawret,nb_class):
    # Check if CUDA is available
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    retlabel = torch.FloatTensor(nb_class,int(rawret.shape[0]/nb_class),int(rawret.shape[1])).to(device)
    cnt1 = 0
    cnt2 = 0
    cnt3 = 0
    cnt4 = 0
